...
Output is checked with `exiftool -validate`. It currently reports four warnings, two of them minor, all from the same limitation: piexifjs omits the four-byte next-IFD terminator on the Exif and GPS sub-IFDs, so the first value in each appears to overlap the directory, and it does not pad between values. Fixing those means re-serialising the TIFF block after piexifjs writes it.
